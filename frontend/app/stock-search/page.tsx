...
export default function StockSearchPage() {
  ...
  async function searchStock(event: React.FormEvent) {
    ...
    try {
      ...
      setStock({
        symbol,
        error: "Unable to search stock. Check the FastAPI backend connection.",
      });
    } finally {
      setLoading(false);
    }
    ...
    <main className="flex min-h-screen bg-slate-950 text-white">
      ...
      <section className="flex-1 px-6 py-8">
        <div className="mx-auto max-w-6xl">
          <div>
            <p className="text-sm font-medium text-blue-400">Stock Search</p>
            <h1 className="mt-1 text-3xl font-bold">Company Research</h1>
            <p className="mt-2 text-slate-400">
              Search a US stock symbol, review fundamentals, valuation metrics,
                  ...
                  {stock.marketmind_score && (
                    <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                      ...
                      <div className="mt-5 space-y-2">
                        {stock.marketmind_score.reasons.map((reason, index) => (
                          <p
                            key={`${reason}-${index}`}
                      ...
                      {stock.marketmind_score.warnings.length > 0 && (
                        <div className="mt-5 rounded-xl border border-yellow-500/20 bg-yellow-500/10 p-4">
                          ...
                          <div className="mt-2 space-y-1">
                            {stock.marketmind_score.warnings.map(
                              (warning, index) => (
                                <p
                                  key={`${warning}-${index}`}
