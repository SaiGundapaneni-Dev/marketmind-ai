...
export default function PortfolioTimeline() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error(
          "Portfolio timeline error:",
          error
        );
        setMessage(
          "Portfolio timeline is unavailable."
        );
      } finally {
        setLoading(false);
      }
  ...
  async function createSnapshot() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          "Unable to create portfolio snapshot."
        );
      }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Track portfolio value, contributions, and
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <MetricCard
              title="Highest Value"
              value={formatMoney(
                performance?.highest_value ?? 0
              )}
              subtitle={`${
                performance?.snapshot_count ?? 0
              } snapshots`}
            />
            ...
            <MetricCard
              ...
              value={formatMoney(
                performance?.worst_day_change ?? 0
              )}
              subtitle="Compared with prior snapshot"
            />
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            ...
            {chartData.length === 0 ? (
              <div className="mt-6 rounded-xl border border-dashed border-slate-700 p-8 text-center text-slate-400">
                Save the first snapshot to begin the
                timeline.
              </div>
            ) : (
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <ContributorPanel
              title="Largest Drags"
              items={
                contributors?.bottom_contributors ?? []
              }
            />
          </div>
          ...
          items.map((item) => (
            ...
            >
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-slate-500">
                  {item.contribution_percent.toFixed(
                    2
                  )}
                  % contribution
                </p>
